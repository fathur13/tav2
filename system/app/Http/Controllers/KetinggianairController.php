...
class KetinggianairController extends Controller
{
    ...
    public function getKIDStatus()
    {
        ...
        if ($kenaikan_air) { // Perlu dilakukan pengecekan apakah data $kenaikan_air ada atau tidak
            ...
            if ($kenaikan_air->ketinggian_air >= 18 && $kenaikan_air->ketinggian_air <= 23) {
                $status = "Bahaya";
                $color = "danger";
            } else if ($kenaikan_air->ketinggian_air >= 12 && $kenaikan_air->ketinggian_air < 18) {
                $status = "Siaga";
                $color = "orange";
            } else if ($kenaikan_air->ketinggian_air >= 6 && $kenaikan_air->ketinggian_air < 12) {
                $status = "Warning";
                $color = "warning";
            } else {
                $status = "Normal";
