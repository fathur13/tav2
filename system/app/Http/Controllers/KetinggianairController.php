...
class KetinggianairController extends Controller
{
    ...
    public function getKIDStatus()
    {
        ...
        if ($kenaikan_air) { // Perlu dilakukan pengecekan apakah data $kenaikan_air ada atau tidak
            ...
            if ($kenaikan_air->ketinggian_air >= 21 && $kenaikan_air->ketinggian_air <= 25) {
                $status = "Bahaya";
                $color = "danger";
            } else if ($kenaikan_air->ketinggian_air >= 14 && $kenaikan_air->ketinggian_air < 21) {
                $status = "Siaga";
                $color = "orange";
            } else if ($kenaikan_air->ketinggian_air >= 7 && $kenaikan_air->ketinggian_air < 14) {
                $status = "Warning";
                $color = "warning";
            } else {
                $status = "Normal";
